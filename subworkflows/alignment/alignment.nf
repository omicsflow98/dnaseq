...
main:
first_ch = Channel.fromPath(params.main.samples, checkIfExists: true)
...
bwa_index = file( "${params.alignment.bwa}/*.{,amb,ann,bwt,pac,sa}" )

runfastqc(first_ch)

if (params.alignment.settings.trim) {
	trim_galore(first_ch)
	trimmed_reads = trim_galore.out.trimmed_fastq
} else {
	trimmed_reads = Channel.fromPath(params.main.samples, checkIfExists: true)
	| splitCsv(header: true, sep: '\t')
	| map { row -> tuple(row.SampName,
			row.LibName,
			row.Barcode,
...
if (params.alignment.settings.recalibrate) {
	BQSR(params.main.reference, params.main.temp_dir, markduplicates.out.mark_dup)
